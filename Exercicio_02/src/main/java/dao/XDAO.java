...
public class XDAO extends DAO {
	...
	public boolean insert(X usuario) {
	...
	public X get(int codigo) {
		...
		try {
	        ...
	        if(rs.next()){            
	        	 usuario = new X(rs.getInt("codigo"), rs.getString("login"), rs.getString("senha"), rs.getString("sexo").charAt(0));
	        }
	        st.close();
		} catch (Exception e) {
	...
	private List<X> get(String orderBy) {	
		...
		List<X> usuarios = new ArrayList<X>();
	...
	public boolean update(X usuario) {
		boolean status = false;
		try {  
			Statement st = conexao.createStatement();
